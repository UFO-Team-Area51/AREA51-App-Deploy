...
app.layout = dbc.Container(html.Div(
    children=[
        ...
        dcc.Store(id='store-data-mbm', data=[], storage_type='memory'),  # 'memory', 'local' or 'session'
        dcc.Store(id='store-data-uet', data=[], storage_type='memory'),  # 'memory', 'local' or 'session'
...
def update_add_remove_agent(n_clicks1, n_clicks2, selected_name_value, options, name_input):
        ...
        if not name_input.isalpha():
            ...
            if not dup_check:
                ...
                with pd.ExcelWriter(selected_name + '.xlsx') as writer:
                    ...
                    mbm_agent_created_msg = {selected_name + '_MBM_Worked': [timestamp], "Action": ['Agent Created']}
                    da_m = pd.DataFrame(mbm_agent_created_msg)
                    uet_agent_created_msg = {selected_name + '_UET_Worked': [timestamp], "Action": ['Agent Created']}
                    da_u = pd.DataFrame(uet_agent_created_msg)
    ...
    return add_agent_msg, rmv_agent_msg, options
...
def update_mbm(submit_n_clicks, button2, button3, options, value, data):
    ...
    if triggered_id == 'mmbm-btn':
        ...
        if not working_true:
            ...
            # Finds agent who has the lowest selection value (last agent who worked case or a new agent)
            prev_assignee = dt.at[0, 'Prev_Agent_MBM']
...
def auto_mbm():
    ...
    if df['Working'].values.sum() != 0:
        ...
        da_m2 = pd.DataFrame(
            {max_agent_name_mbm + '_MBM_Worked': [timestamp], "Action": ['Automatically Assigned Ticket']})
        da_m = pd.concat([da_m1, da_m2])
    ...
    if df.at[min_work_index, 'MBM_Worked'] == 0:
        ...
        if df.at[min_work_index, 'Working']:
            ...
            last_assign_ticket_index = \
                da_mbm_last_agent.loc[da_mbm_last_agent['Action'].str.contains('Assigned Ticket')].index[-1]

            # Replaces value of last "Assigned Ticket" with "Undone" statement and timestamp
            da_mbm_last_agent.iloc[last_assign_ticket_index, da_mbm_last_agent.columns.get_loc(
                'Action')] = 'Case Assignment Undone @ ' + timestamp
...
def auto_uet():
    ...
    if df['Working'].values.sum() == 0:
        ...
        da_u2 = pd.DataFrame(
            {max_agent_name_uet + '_UET_Worked': [timestamp], "Action": ['Automatically Assigned Ticket']})
        da_u = pd.concat([da_u1, da_u2])
...
def undo_uet():
    ...
    if df.at[min_work_index, 'UET_Worked'] == 0:
        ...
        if df.at[min_work_index, 'Working']:
            ...
            # Gets the index value of the last row which contains 'Assigned Ticket' value
            last_assign_ticket_index = \
                da_uet_last_agent.loc[da_uet_last_agent['Action'].str.contains('Assigned Ticket')].index[-1]

            # Replaces value of last "Assigned Ticket" with "Undone" statement and timestamp
            da_uet_last_agent.iloc[last_assign_ticket_index, da_uet_last_agent.columns.get_loc(
...
@app.callback(
    ...
    Output('download_mbm_dropdown', 'options'),
    Output('download_uet_dropdown', 'options'),
    ...
    State('download_uet_dropdown', 'options'),
    State('download_uet_dropdown', 'options'),
    prevent_initial_call=True,
)
def run_report(n_clicks, start_date_mbm, end_date_mbm, start_date_uet, end_date_uet, options_mbm, options_uet):
    ...
    if triggered_id == 'run-report_btn':
        ...
        report_details = 'Finsihed report, there are {} counts of MBM Cases being undone and {} counts of UET ' \
                         'Tickets being undone.'.format(mbm_undo_count, uet_undo_count)
        ...
        return report_details, figure_mbm, figure_uet, options_mbm, options_uet
    else:
        pass
